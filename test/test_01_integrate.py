...
@pytest.mark.parametrize(
    "degree, quad_degree", [(d, q) for q in range(1, 8) for d in range(q + 1)]
)
...
# Test quadrature for polynomials of degree up to 8
@pytest.mark.parametrize(
    "dim, degree, quad_degree",
    [(dim, d, q) for dim in (0, 1) for q in range(1, 8) for d in range(q + 1)],
)
def test_integrate_triangle(dim, degree, quad_degree):
    ...
    analytic = 1.0 / (degree + 1) - 1.0 / (degree + 2)
