...
class BuildTriangle(CellScene):
    def __call__(self, i):
        if i == 0:
            ...
            nodes.append(
                self.anim.caption(
                    "We start with the reference triangle vertices at (0, 0), (0, 1), and (1, 0)"
                )
            )
    ...
    @property
    def frames(self):
        return int(round(4.0 * self.anim.fps))
...
class DrawEdges(CellScene):
    def __call__(self, i):
        if i == 0:
            return (
                self.anim.caption(
                    "The edges are oriented from lower numbered vertex to higher."
                ),
            )
        ...
        if jj < 3:
            ...
            if ii == 0:
                ...
                self.ar.xy = v0 + (0.2 + 0.75 * (ii / self.anim.fps)) * dx
                return (self.ar,)
        elif ii == 0:
            return (
                self.anim.caption(
                    "Notice that that last edge appears to run backwards!"
                ),
            )

    @property
    def frames(self):
        return int(round(10.0 * self.anim.fps))
...
class LabelEdges(CellScene):
    def __call__(self, i):
        if i == 0:
            c = [
                self.anim.caption(
                    "Next we label the edges. Each edge recieves the edge number of the opposite vertex."
                )
            ]
        else:
            c = []
